...
def read_genepop(genepop_file):
	...
	# Regex Fixes (Parsing Genepop) - Y.C.
	pop_catch = r'^\s*(?:Pop|pop|POP)\b'
	sample_catch = r'^\s*([^,]+)\s*,\s*([\d\s]+)$'
	comma_loci_catch = r'^((?:\w+,)+\w+)$'
	...
	for line in genepop_file:
		...
		#Sample capture
			...
			if current_pop == 1:
				names_pop1.append(sample_search.group(1).strip())
				alleles = sample_search.group(2).split()
				allele_list_pop1.append(alleles)
			elif current_pop == 2:
				names_pop2.append(sample_search.group(1).strip())
				alleles = sample_search.group(2).split()
				allele_list_pop2.append(alleles)
...
def build_matrix(raw_alleles, locus_list):
	...
	for i in range(len(raw_alleles[0])):
		...
		for sample in raw_alleles:
			# Python 3 Compatibility fix - Y.C.
